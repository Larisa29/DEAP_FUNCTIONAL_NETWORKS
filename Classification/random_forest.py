...
class RandomForest_classifier(BaseClassifier):
    def __init__(self, path0, path1):
        super().__init__(path0, path1)

    def train_random_forest(self, X_train, y_train):
        """
        Functie responsabila cu antrenarea clasificatorului Random Forest pentru setul de antrenare X_train si clasele corespondente
        y_train.
        """
        random_forest = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=5)
        random_forest.fit(X_train, y_train)
        return random_forest

    def test_random_forest(self, random_forest, X_test, y_test):
        """
        Functie responsabila cu testarea clasificatorului Random Forest pentru setul de antrenare X_test si clasele corespondente
        y_test.
        """
        ...
        print("Clase predicted de random forest: \n", y_pred)
        # compar etichetele obtinute cu cele reale, facand raportul intre nr. predictii corecte/nr total instante
        accuracy = accuracy_score(y_test, y_pred)
        print(classification_report(array_test, y_pred))
        return accuracy, current_confusion_matrix

    def cross_validation_random_forest(self, dataset, number_of_batches, output_file):
        """
        ...
        #creez obiect care va genera subdiviziuni pt. validarea incrucisata
        ...
        # setez optiunea de afisare a intregului dataframe in fisier
        ...
        with open(output_file, 'w') as f:
            ...
            for train_index, test_index in stratified_k_fold.split(X, target): #iterez prin fiecare subdiviziune train-test
                # selectez liniile din dataset corespunzator indicilor din train_index si le atribui variabilelor de antrenare
                ...
                random_forest = self.train_random_forest(X_train_fold, y_train_fold)
                accuracy, current_fold_conf_matrix = self.test_random_forest(random_forest, X_test_fold, y_test_fold)
